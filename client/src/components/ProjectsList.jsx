  ...
  const handleAddTaskSubmit = async () => {
    try {
      const response = await axios.post(
        `https://b4ab214f2633.ngrok.app/projects/${selectedProjectId}/goals/${selectedGoalId}/tasks`,
        {
          name: newTaskName,
        }
      );
      ...
      const goalIndex = updatedProjects[projectIndex].goals.findIndex(
        ...
        {projects.map((project) => (
          <li key={project.id}>
            ...
            {project.goals.map((goal) => (
              <div key={goal.id}>
                {goal.name}
                <button onClick={() => handleAddTaskClick(project.id, goal.id)}>
                  Add Task
                </button>
                {showAddTask &&
